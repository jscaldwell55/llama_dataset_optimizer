...
from core.deduplicator import get_llama_embeddings, deduplicate_faiss_gpu
...
def main(args):
    ...
    # We need a tokenizer to normalize, so let's load a base one first
    ...
    # These steps reduce the dataset size before the expensive scoring step.

    # 3. Quality Filtering
    quality_passed_indices = batch_quality_check(original_dataset, config, base_tokenizer)
    ...
    print(f"Dataset size after filtering & deduplication: {len(deduplicated_dataset)}")
    ...
    if args.top_k > len(final_scores):
        print(f"Warning: top_k ({args.top_k}) is larger than the number of available samples ({len(final_scores)}). Using all samples.")
    ...
    print(f"Selected top {args.top_k} samples based on combined scores.")
    ...
    if args.validate:
        if not args.test_model:
            print("Warning: --validate flag is set but --test-model is not specified. Skipping validation.")
        else:
